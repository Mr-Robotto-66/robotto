...
## Protected Layers (What They Mean and How They Are Created)

"Protected layers" in this context are the base BCGW layers used to screen out satellite
cutblocks that overlap heavily with protected or restricted areas. In
`CC_Step_1_create_layer.py`, these are built from BCGW datasets and clipped to the AOI
using `select_cutblocks()` before the satellite overlay check runs.

Exact BCGW datasets used:

- Forest ownership: `WHSE_FOREST_VEGETATION.F_OWN`
  - Filtered with `bcgw_f_own_def_query` to keep specific ownership/schedule codes
- Provincial parks: `WHSE_TANTALIS.TA_PARK_ECORES_PA_SVW`
- NGO conservation areas: `WHSE_LEGAL_ADMIN_BOUNDARIES.WCL_CONSERVATION_AREAS_NGO_SP`
- National parks: `WHSE_ADMIN_BOUNDARIES.CLAB_NATIONAL_PARKS`
- Indian reserves: `WHSE_ADMIN_BOUNDARIES.CLAB_INDIAN_RESERVES`

Creation steps in the script:

- Each layer is selected to the AOI using `select_cutblocks(aoi_mem, ...)`.
- `F_OWN` uses `bcgw_f_own_keep_flds` and `bcgw_f_own_def_query` to limit attributes and
  include only the allowed ownership/schedule codes.
- The resulting in-memory feature classes are collected into `base_layers` and passed to
  `process_landsat_overlay()`, which calculates overlap and excludes satellite blocks with
  **>= 75%** overlap against any of these protected/ownership layers.
